...
Интерфейс: `docs/LIVE_AI_CONTRACT.md`. Модель та же (faster-whisper large-v3 int8, CPU), загружается один раз. Симуляция без UI:

```bash
cd backend
MODELS_DIR=../models ASR_COMPUTE_TYPE=int8 .venv/bin/python ../scripts/ai/live_simulate.py \
    tests/fixtures/ai/meeting_ru_kk.wav [--realtime] [--final]
MODELS_DIR=../models ASR_COMPUTE_TYPE=int8 .venv/bin/python -m pytest tests/ai/test_live_real.py -s   # 4 passed, 3 мин 45 с
.venv/bin/python -m pytest tests/ai/test_live_merge.py                                                # без моделей
```

Симулятор кодирует WAV в один WebM/Opus-контейнер (кластеры по 1 с, как MediaRecorder) и режет байты на части по ~2 с. Каждые 4 с аудио он передаёт в preview зафиксированную копию префикса с обрезанным хвостом. С `--realtime` части поступают по настенным часам, а лаг = доступное аудио − `processed_until_seconds`.

Измерения на CPU M4 Pro (ASR large-v3 int8):

| Конфигурация | Стоимость вызова ASR | Лаг на записи 113 с (реальное время) | Итог |
| --- | --- | --- | --- |
| окно 12 с, beam 5, без переопределения языка | 11–14 с на ≤ 12 с аудио | растёт: 14 → 29 → 45 → 48 с | не успевает |
| окно 12 с, beam 1 | 11–12 с | — | почти без выигрыша, текст хуже |
| **окно 24 с, beam 5, без переопределения** (дефолт) | 12–18 с на окно, до 64 с при 3 окнах | 14–31 с, пик 62 с, к концу 0 | успевает с задержкой |
| окно 24 с, с переопределением языка по сегментам | до 147 с на вызов | до 87 с | не успевает на CPU |

Почему так: faster-whisper кодирует блок в 30 с при любой длине окна, поэтому вызов стоит ~12 с даже на 4 с аудио. Реальное время обеспечивают только окна, почти целиком заполняющие этот блок. Первый текст появляется через ~14–18 с после начала (после первых 4 с аудио плюс ~12 с ASR). Декодирование снимка FFmpeg < 0.1 с.

Качество preview на смоделированных фикстурах:
- Русские реплики совпадают с эталоном. Нет дублей на перекрытиях, повторы тех же фраз в разное время сохраняются, таймкоды общие и упорядоченные, тишина не создаёт реплик.
- **Казахские фразы внутри русского окна без переопределения языка транслитерируются** («Салеметсиздерми…»), а иногда **переводятся/перефразируются на русский** («Сегодня в журнале…» вместо «Бүгінгі жиналыста…»). Короткие KZ-фразы («Презентацияны кім жасайды?») могут пропасть. Финальная обработка с `ASR_REFINE_LANGUAGES=true` это исправляет (§8). В UI preview надо помечать как черновой.
- Исправленные при разработке дефекты: окно, начатое внутри слова, давало галлюцинацию «Продолжение следует...» на всё окно (начало окна теперь в тихой точке + фильтр фраз); слово после паузы терялось из-за растянутого назад таймкода Whisper (теперь решает конец слова); речь, которую модель не распознала у края окна, пропускалась как «пауза» (теперь пропускается только тишина по энергии).

Финализация: `live_simulate.py --final` и `test_finalization_uses_full_pipeline` подают полный контейнер в `process_recording`. Результат: `normalizing → transcribing → diarizing`, затем `MODEL_UNAVAILABLE (model=diarization)` — gated-весов нет, заглушки нет.

Не сделано: P1 (предварительные поручения), live-диаризация. Не проверено: живой микрофон в браузере и настоящие чанки MediaRecorder (Chrome/Safari). Сейчас контейнер собран FFmpeg, и его кластеризация может отличаться.
