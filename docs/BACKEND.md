...
## Удаление (архивирование) сотрудника

`DELETE /employees/{id}` (admin, secretary; CSRF) переиспользует `employees.active`, схема не менялась. Одна транзакция: блокировка активных admin (защита `LAST_ADMIN` от гонки) и строки сотрудника `FOR UPDATE` → `active=false`, учётная запись `active=false`, все сессии удалены. `get_current_user` дополнительно отвергает сессию, если сотрудник архивирован. Каскадов нет: встречи, участники, спикеры, поручения, голосовой профиль остаются; поручения не закрываются и не переназначаются.

Защита от гонок: `POST /users` блокирует строку сотрудника и отклоняет архивированного (`409 EMPLOYEE_ARCHIVED`); регистрация голоса проверяет `active` до и повторно под блокировкой после вычисления вектора; выбор участников встречи читает сотрудников `FOR SHARE` (архивирование ждёт или проверка падает с `422`); `PATCH /users {active:true}` для архивированного — `409 EMPLOYEE_ARCHIVED`. Кандидаты сопоставления голоса — только активные сотрудники. `can_delete` вычисляется сервером (`serializers.delete_denial`), сервер проверяет права независимо.

## Live-запись из браузера

Контракт: `docs/LIVE_CONTRACT.md` (HTTP), `docs/LIVE_AI_CONTRACT.md` (Python, AI-агент: `app/live_ai.py`, `app/live_ai_types.py`). Код: `app/live_routes.py`, `app/live_service.py`, миграция `0003` (`live_sessions`, `meetings.meeting_url`).

- Чанки `PUT …/chunks/{sequence}` дописываются строго по порядку в один файл `DATA_DIR/live/<meeting>/<session>/stream.<ext>`; перед записью файл обрезается до закоммиченного `received_bytes`, после записи — `fsync`, затем commit и ACK. sha256 каждого чанка хранится для идемпотентных повторов (`CHUNK_CONFLICT`, `CHUNK_OUT_OF_ORDER`).
- Одна сессия `recording` на встречу (частичный уникальный индекс + проверка).
- Preview: после чанка, если накопилось ≥ `LIVE_PREVIEW_MIN_NEW_BYTES` новых байт, ставится `run_preview`; на сессию работает не больше одного цикла, на процесс — одно preview-задание (семафор), новые байты подхватываются тем же циклом (очередь не растёт). Во время полной обработки preview не запускается и прерывается через `is_cancelled`. AI получает копию стабильного префикса контейнера; ошибка preview → `preview_status=unavailable`, запись продолжается. Последний результат AI (с его инкрементальным состоянием) хранится в памяти процесса; после рестарта `previous` восстанавливается из сохранённых реплик.
- `finish` проверяет все чанки и передаёт файл в существующий `_store_recording` (ffprobe, лимиты, замена прежней записи/черновика, `run_recording_job` → полный `process_recording`). Повторный `finish` возвращает тот же `recording_id`.
- Рестарт: `recording` → `error INTERRUPTED`; тик планировщика: нет чанков дольше `LIVE_IDLE_TIMEOUT_SECONDS` → `error CLIENT_TIMEOUT`. В обоих случаях байты сохранены и сессию можно завершить `finish` или отменить. `cancel` удаляет каталог сессии, прежний протокол не трогает.
- `meeting_url` хранится как текст (только http/https), сервер по нему не ходит.
...
- Архивирование не удаляет аудио и голосовые векторы физически; повторная активация сотрудника (`PATCH /employees {active:true}`, admin) не реактивирует учётную запись автоматически. Задание, уже начавшее обработку, использует снимок кандидатов на момент старта.
